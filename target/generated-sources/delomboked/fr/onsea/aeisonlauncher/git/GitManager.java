// Generated by delombok at Sat Feb 25 14:31:36 CET 2023
...
public class GitManager {
	 ...
	 * @throws Exception
	 */
	public static final Git cloneContent(final String urlIn, final String branchNameIn, final File localDirectoryFileIn) throws Exception {
		if (!localDirectoryFileIn.exists()) {
			if (GitManager.WITH_MONITORING) {
				System.out.println("\"" + localDirectoryFileIn.getAbsolutePath() + "\" directory does not exist, try to create it");
			}
			localDirectoryFileIn.mkdirs();
		}
		final var cloneCommand = Git.cloneRepository().setURI(urlIn).setBranch(branchNameIn);
		if (GitManager.WITH_MONITORING) {
			System.out.println("Attempts to clone the branch \"" + branchNameIn + "\" from repository \"" + urlIn + "\" into \"" + localDirectoryFileIn.getAbsolutePath() + "\"");
	 ...
	 * @throws Exception
	 */
	public static final Git initRepository(final File localDirectoryFileIn, final String urlIn, final String branchNameIn) throws Exception {
		final var localRepo = GitManager.local(localDirectoryFileIn.getAbsolutePath());
		if (GitManager.WITH_MONITORING) {
			System.out.println("Attempts to init from \"" + localDirectoryFileIn.getAbsolutePath() + "\" with branch \"" + branchNameIn + "\" of \"" + urlIn + "\" repository");
		}
		var pull = false;
		Git git = null;
	...
	private final File localDirectoryFile;
	...
	public GitManager(final String urlIn, final String directoryIn) throws Exception {
		...
		this.git = GitManager.initRepository(this.localDirectoryFile, urlIn, this.branchName);
	}

	public final GitManager init() throws Exception {
		this.git = GitManager.initRepository(this.localDirectoryFile, this.url, this.branchName);
		return this;
	}
	...
	public File getLocalDirectoryFile() {
		return this.localDirectoryFile;
	}
